...
void LoginLogoutTracker::TrackEvent() {
    ...
    while (ReadEventLog(hEventLog, EVENTLOG_SEQUENTIAL_READ | EVENTLOG_FORWARDS_READ,
        ...
        while ((BYTE*)pRecord < (BYTE*)pBuffer + dwBytesRead) {
            // Olay ID'sini kontrol et (login ID'si: 4624, logout ID'si: 4634)
            if (pRecord->EventID == 4624 || pRecord->EventID == 4634) {
                ...
                std::cout << "Readable Time: " << readableTime << std::endl;
